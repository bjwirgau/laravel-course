...
<div class="rounded-lg shadow-md bg-white p-4">
    ...
    <ul class="my-4 bg-gray-100 p-4 rounded">
        <li class="mb-2"><strong>Salary:</strong> {{Number::currency($job->salary, 'USD', 'en_US', 0)}}</li>
        <li class="mb-2">
            <strong>Location:</strong> {{$job->city}}, {{$job->state}}
            <span class="text-xs {{ $job->remote ? 'bg-green-500' : 'bg-red-500' }} text-white rounded-full px-2 py-1 ml-2">{{ $job->remote ? 'Remote' : 'On-site' }}</span>
        </li>
        @if ($job->tags)
            <li class="mb-2">
